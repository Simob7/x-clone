  ...
  return (
    ...
    <div className="min-h-screen flex items-center justify-center p-8 bg-black text-white">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between w-full max-w-7xl gap-12">
        ...
        {/* RIGHT SECTION: Registration UI */}
        <div className="w-full lg:w-1/2 flex flex-col justify-center items-center lg:items-start gap-4">
          <h1 className="text-4xl xsm:text-5xl md:text-7xl font-bold tracking-tighter mb-4 text-center lg:text-left">
            Happening now
          </h1>
          ...
          {/* ROOT: Manages the registration state machine */}
          <SignUp.Root>
            {/* STEP: START - Initial user data collection */}
            <SignUp.Step name="start" className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-4">
                ...
                <Clerk.Field name="username" className="flex flex-col gap-2">
                  <Clerk.Input
                    className="py-2 px-6 rounded-full text-black w-72 placeholder:text-sm outline-none border focus:border-iconBlue transition"
                ...
                <SignUp.Action
                  submit
                  className="bg-iconBleu rounded-full p-2 text-white font-bold w-72 text-center hover:opacity-90 transition cursor-pointer">
                  Sign up
                </SignUp.Action>
              </div>
            </SignUp.Step>

            {/* STEP: CONTINUE - Handles missing info if OAuth login didn't provide it */}
            <SignUp.Step name="continue" className="flex flex-col gap-4">
              <Clerk.Field name="username">
                <Clerk.Input
                  placeholder="Finish your username"
                  className="py-2 px-6 rounded-full text-black w-72 placeholder:text-sm outline-none"
                />
                <Clerk.FieldError className="text-red-400 text-sm" />
              </Clerk.Field>
              ...
              <SignUp.Action
                submit
                className="w-72 text-center text-iconBlue underline cursor-pointer">
                Continue
              </SignUp.Action>
            </SignUp.Step>

            {/* STEP: VERIFICATIONS - OTP Email verification flow */}
            <SignUp.Step name="verifications">
              <SignUp.Strategy name="email_code">
                <h3 className="text-lg font-bold mb-2">Check your e-mail</h3>
                <p className="text-xs text-gray-400 mb-4 w-72">
                ...
                <SignUp.Action
                  submit
                  className="mt-4 bg-white text-black font-bold py-2 rounded-full w-72 text-center cursor-pointer hover:bg-gray-200 transition">
                  Verify Account
                </SignUp.Action>
              </SignUp.Strategy>
            </SignUp.Step>
            ...
            {/* REDIRECT TO SIGN IN */}
            <div className="w-72 flex flex-col items-center mt-6">
              ...
              <Link
                href="/sign-in"
                className="border bg-iconBleu border-borderGray rounded-full p-2 text-iconBlue font-bold w-72 text-center hover:bg-[#1d9bf01a] transition">
                Already have an account?
              </Link>
            </div>

            {/* FOOTER: Legal and Policy */}
            <p className="w-72 text-xs text-gray-500 mt-6 leading-tight">
